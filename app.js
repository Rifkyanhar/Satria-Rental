...
if(trySetupSupabase()){
  ...
  // Mungkin CDN utama gagal dan sedang memuat CDN cadangan — coba lagi beberapa kali.
  let tries = 0;
  const retryTimer = setInterval(()=>{
    tries++;
    if(trySetupSupabase() || tries >= 10){
      clearInterval(retryTimer);
      if(sb){
        attachAuthStateListener();
      } else {
        const showErr = ()=>{ const el = document.getElementById('authError'); if(el){ el.style.color = 'var(--alert)'; el.textContent = 'Gagal memuat koneksi ke server. Cek internet HP Anda lalu refresh halaman.'; } };
...
function withTimeout(promise, ms, label){
  return Promise.race([
    promise,
    new Promise((_, reject)=> setTimeout(()=> reject(new Error(`${label} — waktu habis (koneksi ke server terlalu lama, cek internet Anda)`)), ms))
  ]);
}
...
window.handleSignup = async function(){
  ...
  const originalText = btn.textContent; btn.textContent = 'Memproses...'; btn.disabled = true;
  try{
    const { error } = await withTimeout(sb.auth.signUp({ email, password }), 8000, 'Daftar');
...
window.handleForgotPassword = async function(){
  ...
  if(!sb){ errEl.style.color='var(--alert)'; errEl.textContent='Koneksi ke server belum siap. Refresh halaman dan coba lagi.'; return; }
  errEl.style.color = 'var(--alert)'; errEl.textContent = '';
  const email = document.getElementById('forgotEmail').value.trim();
  if(!email){ errEl.textContent = 'Isi email dulu.'; return; }
  const btn = document.querySelector('#forgotBox .btn-primary');
  const originalText = btn.textContent; btn.textContent = 'Mengirim...'; btn.disabled = true;
  try{
    const redirectTo = window.location.origin + window.location.pathname;
    const { error } = await withTimeout(sb.auth.resetPasswordForEmail(email, { redirectTo }), 8000, 'Kirim link reset');
    if(error){
      errEl.textContent = 'Gagal kirim: ' + error.message;
    } else {
      errEl.style.color = 'var(--go)';
      errEl.textContent = 'Link reset sudah dikirim. Cek inbox (dan folder spam) email Anda, lalu klik link di dalamnya.';
    }
  }catch(e){
...
window.handleResetPassword = async function(){
  ...
  const btn = document.querySelector('#resetScreen .btn-primary');
  const originalText = btn.textContent; btn.textContent = 'Menyimpan...'; btn.disabled = true;
  try{
    const { error } = await withTimeout(sb.auth.updateUser({ password }), 8000, 'Simpan password baru');
    if(error){
    ...
    errEl.style.color = 'var(--go)';
    errEl.textContent = 'Password berhasil diubah. Mengarahkan ke aplikasi...';
    setTimeout(async ()=>{
  ...
  return `
    <div class="list-card">
      ...
      <div class="list-card-actions">
        <button class="btn btn-ghost" onclick="printSingleCustomer('${c.id}')">🖨 Cetak</button>
...
window.printSingleCustomer = function(id){
  const c = STATE.customers.find(x=>x.id===id);
  if(!c){ alert('Data penyewa tidak ditemukan.'); return; }
  const s = STATE.settings;
  const flagged = STATE.blacklist.find(b=>b.nik===c.nik);
  const custContracts = STATE.contracts.filter(k=>k.customer_id===c.id);
  const riwayatRows = custContracts.length
    ? custContracts.map(k=>{
        const u = STATE.units.find(x=>x.id===k.unit_id);
        return `<tr>
          <td>${u?u.model+' ('+u.plat+')':'-'}</td>
          <td>${fmtDate(k.tgl_mulai)} — ${fmtDate(k.tgl_selesai)}</td>
          <td>${fmtMoney(k.deposit)}</td>
          <td>${k.lunas?'Lunas':'Belum Lunas'}</td>
        </tr>`;
      }).join('')
    : `<tr><td colspan="4">Belum ada riwayat sewa tercatat.</td></tr>`;

  const win = window.open('', '_blank');
  win.document.write(`
    <html><head><title>Data Penyewa - ${c.nama} - ${s.business_name||'Satria Rental'}</title>
    <style>
      body{font-family:Arial, sans-serif; padding:24px; color:#111;}
      h1{font-size:18px; margin-bottom:2px;}
      h2{font-size:15px; margin:18px 0 8px;}
      .sub{color:#555; font-size:12px; margin-bottom:20px;}
      .flag-banner{background:#fdeaea; border:1px solid #b02a2a; color:#b02a2a; padding:10px 14px; border-radius:6px; font-size:13px; margin-bottom:18px; font-weight:bold;}
      .print-table{width:100%; border-collapse:collapse; font-size:12px; margin-bottom:8px;}
      .print-table td{padding:5px 8px; vertical-align:top; border-bottom:1px solid #eee;}
      .print-table td:first-child{width:150px; color:#666;}
      .hist-table{width:100%; border-collapse:collapse; font-size:12px; margin-bottom:8px;}
      .hist-table th, .hist-table td{padding:5px 8px; text-align:left; border-bottom:1px solid #eee;}
      .hist-table th{color:#666; font-weight:normal; border-bottom:1px solid #999;}
      .print-photos{display:flex; gap:14px; margin-top:6px;}
      .print-photos img{max-width:220px; max-height:150px; object-fit:contain; border:1px solid #ccc;}
      .print-photos small{display:block; color:#666; font-size:10px; margin-bottom:2px;}
      .footer-note{margin-top:30px; font-size:11px; color:#777; border-top:1px solid #ccc; padding-top:10px;}
      @media print{ body{padding:0;} }
    </style>
    </head><body>
    <h1>${s.business_name||'Satria Rental'} — Laporan Data Penyewa</h1>
    <div class="sub">Dicetak: ${fmtDate(todayISO())}</div>
    ${flagged ? `<div class="flag-banner">⚠ PERINGATAN: NIK penyewa ini tercatat dalam Blacklist Internal. Alasan: ${flagged.alasan} (dicatat ${fmtDate(flagged.tgl_input)})</div>` : ''}

    <h2>Data Diri</h2>
    <table class="print-table">
      <tr><td>Nama Lengkap</td><td>${c.nama}</td></tr>
      <tr><td>NIK</td><td>${c.nik||'-'}</td></tr>
      <tr><td>No. HP</td><td>${c.hp||'-'}</td></tr>
      <tr><td>Alamat</td><td>${c.alamat||'-'}</td></tr>
      <tr><td>Terdaftar Sejak</td><td>${fmtDate(c.created_at)}</td></tr>
      <tr><td>Verifikasi Wajah</td><td>${c.face_match_score!=null ? (c.face_verified?'Cocok':'Perlu Dicek Manual')+' ('+Math.round(c.face_match_score*100)+'%)' : 'Belum dicek'}</td></tr>
    </table>

    <h2>Riwayat Sewa</h2>
    <table class="hist-table">
      <tr><th>Unit</th><th>Periode</th><th>Jaminan</th><th>Status</th></tr>
      ${riwayatRows}
    </table>
    ...
    <div class="footer-note">Dokumen ini dicetak otomatis dari sistem manajemen armada ${s.business_name||'Satria Rental'} sebagai catatan internal / lampiran laporan. Data ini bersumber dari database yang diisi oleh pihak rental saat pendaftaran penyewa.</div>
    </body></html>
  `);
  win.document.close(); win.focus();
  setTimeout(()=>win.print(), 300);
