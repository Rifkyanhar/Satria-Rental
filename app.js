...
window.handleLogin = async function(){
  ...
  if(error){ authError.textContent = 'Gagal masuk: ' + error.message; return; }
  await enterApp();
};
...
window.handleSignup = async function(){
  ...
  if(error){ authError.textContent = 'Gagal daftar: ' + error.message; return; }
...
window.openCustomerForm = function(id){
  ...
  openModal(`
    ...
    <div class="field">
      ...
      <input type="file" accept="image/*" id="cSelfieFile">
      <img id="cSelfiePrev" class="file-preview" src="${c.selfie_foto||''}" style="${c.selfie_foto?'display:block':''}">
    </div>
    <button class="btn btn-primary btn-block" id="cSaveBtn" onclick="saveCustomer('${id||''}')">Simpan</button>
